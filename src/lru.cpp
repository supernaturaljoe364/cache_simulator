...
void LRU::put(const std::string& key, const int& value){
...
std::optional<int> LRU::get(const std::string& key){
...
bool LRU::contains(const std::string& key){
  data.totalRequests++;
  return check.find(key) != check.end();
}

void LRU::displayStats() const {
